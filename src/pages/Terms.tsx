  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-3xl px-4 pt-28 pb-16 md:pt-32">
        ...
        <p className="text-muted-foreground text-center mb-10">{t("terms.updated")}</p>

        <div className="space-y-8 text-muted-foreground leading-relaxed">
          {(["s1", "s2", "s3", "s4"] as const).map((s) => (
            <section key={s} className="glass rounded-xl p-6">
              <h2 className="text-lg font-semibold text-foreground mb-3">{t(`terms.${s}title` as any)}</h2>
              <p>{t(`terms.${s}text` as any)}</p>
            </section>
          ))}
        </div>
      </div>
